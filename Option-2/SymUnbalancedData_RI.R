SymUnbalancedData_RI<- function(DATA_GROUP,PRE_SET_MAX,STEP_SIZE,FROM )
{
...
for(j in 1:PRE_SET_MAX) {
    if (j <= n_i)
       { 
            ...
            UNBALANCED_CD4[i,j]<- ((B.Intercept + b.rand[1] ) + (B.Time *TimePoints[j]) + (B.Xi*Trt[i,j]) + (B.X.time*Trt[i,j]*TimePoints[j])+ Errors)
            TRTART[i,j] <-Trt[i,j]
