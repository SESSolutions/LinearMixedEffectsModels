SymBalancedData_RI<-function(N_Data_groups,FollowUp_Times){
...
for(i in 1:N_Data_groups){
  ...
  for(j in 1:N_Measures){
     Errors <- (0.5*rnorm(1, mean=1.9, sd=0.6)+ 0.5*rnorm(1, mean=-1.9, sd=0.6))  
     Week[i,j] <- FollowUp_Times[j]
     PatId[i,j] <- i
     LikeForLike.Response[i,j] <- ((B.Intercept + b.rand[1]) + (B.Time*FollowUp_Times[j])+ (B.Xi*discrete.covariate[i,j])+ (B.X.time*FollowUp_Times[j]*discrete.covariate[i,j])+Errors)# equation 1       
        }                       
 }
